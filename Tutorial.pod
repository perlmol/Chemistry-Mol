...
the C<read_mol> subroutine. The third line reads the file and returns a Mol object.
...
A bond object is a list of atoms. In most cases, a bond has exactly two atoms,
but we don't want to exclude possibilities such as three-center bonds. You can
get the list of atoms in a bond by using the C<atoms> method.
...
However, one of the interesting things of object-oriented programming is that
...
some added functionality. In fact, the PDB reader normally returns MacroMol,
not Mol, objects, but in the preceding examples we assumed that they were
Mol objects with no problem at all. Now the question is, what is the 
"added functionality" that MacroMol objects have on top of the original 
Mol object?
